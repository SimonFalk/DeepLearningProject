...
depth      = int(inputs[0])
width      = int(inputs[1])
depth_post = int(inputs[2])
width_post = int(inputs[3])
dropout    = float(inputs[4])
identifier = int(inputs[5])
...
# Mid
model.add(layers.Lambda(lambda inputs: inputs[:, -p_out:, :]))
...
with open(f'results_batch_test_3/res-{identifier}.pickle', 'wb') as fp:
    pickle.dump({
        'mse': mse,
